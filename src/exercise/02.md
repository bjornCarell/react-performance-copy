...
To have a nice user experience we need to hit 60 frames a second, that's 16
milliseconds per frame. 1000 / 60.

Learn more about Web Workers.

Main take away for now: We can take some JavaScript that is expensive to run and
put it on a separate thread so that our main thread is freed up to display
things to the user more rapidly.
