...
If we have calculations that are triggering changes to the DOM on changes,
see if it is possible to move those calculations to be calculated higher 
up in the tree. In that way you may be able to pass the calculations in props,
as primitives, and you do not need to write complex logic for comparison.
## Background
